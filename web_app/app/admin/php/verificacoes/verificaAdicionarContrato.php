...
// Terceira Parte
    ...
    $idColaboradorContrato = $_POST['adicionarNomeContrato'];

    $query = "UPDATE contrato SET atual = 0 WHERE ref_id_colaborador = ?";
    $stmt = mysqli_prepare($link, $query);
    mysqli_stmt_bind_param($stmt, 's', $idColaboradorContrato);
...
vencimento_base = ?, data_admissao = ?, data_final_contrato = ? WHERE id_colaborador = ?";
        ...
        header('location:../../adicionarContrato.php?sucesso=' . $idColaboradorContrato);
    } else {
        mysqli_stmt_fetch($stmt);
        mysqli_stmt_close($stmt);
        header('location:../../adicionarColaborador.php?sucesso=' . $idColaboradorContrato);
    }
}