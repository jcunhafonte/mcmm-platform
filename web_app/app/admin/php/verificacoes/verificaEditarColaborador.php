...
// Terceira Parte
    ...
    $query = "UPDATE colaborador SET tipo_admissao = ?, nome_completo = ?,
morada = ?, localidade = ?, codigo_postal = ?, telefone = ?, telemovel = ?,
email = ?, nascimento = ?, naturalidade = ?, nacionalidade = ?,
cartao_cidadao = ?, emissao_cartao_cidadao = ?, entidade_emissora = ?, validade_cc_passaporte = ?,
contribuinte_passaporte = ?, seg_social = ?, carta_conducao = ?,
validade_conducao = ?, categoria_conducao = ?, situacao_militar = ?,
estado_civil = ?,deficiencia_colaborador = ?, conjuge_colaborador = ?,
contribuinte_conjuge = ?, titulares_rendimento = ?, familiares_cargo = ?,
numeros_filhos = ?, idades_filhos = ?, deficiencia_filhos = ?,
reg_seg_social = ?, percentagem_funcionario = ?, percentagem_patronal = ?,
funcao = ?, categoria = ?, situacao_contratual = ?, posto_trabalho = ?,
periodo_experimental = ?, data_final_per_experimental = ?,
horas_semanais = ?, horas_diarias = ?, descanso_complementar = ?,
sistema_rotativo = ?, NIB = ?, instituicao_bancaria = ?, agencia = ?,
vencimento_base = ?, data_admissao = ?, escolaridade = ?, data_final_contrato = ?
WHERE id_colaborador = ?";

    $stmt = mysqli_prepare($link, $query);

    mysqli_stmt_bind_param($stmt, 'sssssiissssisssiisssssssisiissiiissssisiississssssi',
        ...
        $vencBaseColaborador, $novaDataAdmissao, $escolaridadeColaborador, $novaDataFinalContratoColaborador,
        $idColaborador);
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        $query = "SELECT id_contrato FROM contrato WHERE ref_id_colaborador = ? AND atual = 1 LIMIT 1";
        $stmt = mysqli_prepare($link, $query);
        mysqli_stmt_bind_param($stmt,'i', $idColaborador);
        mysqli_stmt_bind_result($stmt, $idColaboradorContratoColab);
...
vencimento_base = ?, data_admissao = ?, ref_id_colaborador = ?, data_final_contrato = ? WHERE id_contrato = ?";

        $stmt = mysqli_prepare($link, $query);

        mysqli_stmt_bind_param($stmt, 'ssisssiississssisi', $situacaoContratualColaborador, $postoTrabalhoColaborador,
            ...
            $sistemaRotativoColaborador, $NIBColaborador, $isntBancariaColaborador, $agenciaColaborador,
            $vencBaseColaborador, $novaDataAdmissao, $idColaborador, $novaDataFinalContratoColaborador, $idColaboradorContratoColab);

        if (mysqli_stmt_execute($stmt)) {
            mysqli_stmt_fetch($stmt);
            mysqli_stmt_close($stmt);
            header('location:../../editarPerfil.php?colaborador='.$idColaborador.'&sucesso=1');
        } else {
            mysqli_stmt_fetch($stmt);
            mysqli_stmt_close($stmt);
            header('location:../../editarPerfil.php?colaborador='.$idColaborador.'&sucesso=2');
        }
    } else {
        mysqli_stmt_fetch($stmt);
        mysqli_stmt_close($stmt);
