...
Menus('descricao','registoDescricoes');
...
if (isset($_GET['descricaoRemovida'])) {
    $nomeFuncao = $_GET['descricaoRemovida'];
    echo "<div class='alert alert-success alert-dismissable'>
                <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>
               A descrição da função <b>$nomeFuncao</b> foi removida com com sucesso.<br>
               <a class='alert-link'>Votos de uma excelente experiência!</a></div>";
}

if (isset($_GET['descricaoRemovidaErro'])) {
    $nomeFuncao = $_GET['descricaoRemovidaErro'];
    echo "<div class='alert alert-danger alert-dismissable'>
                <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>
               Ocorreu um erro ao remover a descrição da função <b>$nomeFuncao</b>.<br>
               <a class='alert-link'>Tente novamente!</a></div>";
}

$query = "SELECT titulo, objetivo, enquadramento, id_descricao FROM descricao ORDER BY titulo ASC";
...
if (mysqli_stmt_num_rows($stmt) > 0) {
    ...
    echo "<div class='widget'>
                        <div class='widget-header'>
                            <h2>Consultar <strong>Descrições</strong></h2>
                        ...
                        <div class='widget-content padding'>
                            <table id='user' class='table table-bordered table-striped' style='clear: both'>
                                <tbody>
                                 <thead>
                                        <tr>
                                            <th>Título de Função</th>
                                            <th>Objetivo</th>
                                            <th>Enquadramento Orgânico</th>
    ...
    $stmt = mysqli_prepare($link, $query);
    mysqli_stmt_bind_result($stmt, $tituloDescricao, $objetivoDescricao, $enquadramentoDescricao, $idDescricao);
    mysqli_stmt_execute($stmt);

    while (mysqli_stmt_fetch($stmt)) {

        echo "<tr>
                                    <td width='15%' id='titulo$idDescricao'>$tituloDescricao</td>
                                    <td width='35%'>$objetivoDescricao</td>
                                      <td width='35%'>$enquadramentoDescricao</td>
                                    <td width='15%' class='text-center'>


                                        <div class='btn-group btn-group-xs text-center' style='padding-top: 20px!important;'>
                                                <a
                                                href='descricaoDetalhes.php?descricao=$idDescricao'
                                                data-toggle='tooltip' title='Detalhes - $tituloDescricao'
                                                class='text-center btn btn-default'>
                                                <i class='fa fa-eye fa-1x'></i></a>

                                                <a
                                                href='descricaoDetalhesImprimir.php?descricao=$idDescricao'
                                                data-toggle='tooltip' title='Imprimir - $tituloDescricao'
                                                class='text-center btn btn-default'>
                                                <i class='fa fa-print fa-1x'></i></a>
                                                ...
                                                onclick='modalRemover($idDescricao);'
                                                  ...
                                                  data-toggle='tooltip' title='Remover Descrição - $tituloDescricao'
                                                  data-target='#md-fade-in-scale-up' class='text-center btn btn-default md-trigger'
    ...
    echo "<div class='widget'>
        ...
        echo "<tr>
                                    <td width='100%' colspan='4'>Não há descrições de funções para apresentar.
                                    Dirija-se à area de Adicionar Descrições para ter a possibilidade de consultar esta área.</td>
