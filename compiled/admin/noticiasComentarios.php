...
<div id="wrapper">
    ...
    Menus('noticias', 'comentariosNoticias');
                            ...
                            <div class="table-responsive" style="overflow-x: hidden">
                                <form class='form-horizontal' role='form'>
                                    <table id="datatables-1" class="table table-striped table-bordered" cellspacing="0"
                                        ...
                                        $ativo = 1;
                                        $query = "SELECT comentarios_noticias.id_comentarios_noticias, comentarios_noticias.comentario, 
                                        comentarios_noticias.ref_id_utilizador, comentarios_noticias.ref_id_noticia, 
                                        comentarios_noticias.data_comentario, utilizadores.nome_utilizador, utilizadores.id_utilizador, 
                                        utilizadores.id_user, noticias.id_noticias, noticias.ativo, noticias.titulo
                                        FROM comentarios_noticias
                                        INNER JOIN utilizadores ON utilizadores.id_utilizador = comentarios_noticias.ref_id_utilizador
                                        INNER JOIN noticias ON noticias.id_noticias = comentarios_noticias.ref_id_noticia
                                        WHERE noticias.ativo = ?
                                        ORDER BY comentarios_noticias.data_comentario";

                                        $stmt = mysqli_prepare($link, $query);
                                        mysqli_stmt_bind_param($stmt, 'i', $ativo);
                                        mysqli_stmt_bind_result($stmt, $idComentariosNoticias, $comentario, $refIdUtilizador, $refIdNoticia,
                                            $dataComentario, $nomeUtilizador, $idUtilizador, $idUser, $idNoticias, $ativo, $titulo);
                                        ...
                                        while (mysqli_stmt_fetch($stmt)) {
                                            ...
                                            <td>
                                                <a href='/@$idUser'>
                                                <span id='titulo$idComentariosNoticias'>"  .  htmlentities($nomeUtilizador)  . "</a>
                                                </a>
                                            </td>
                                            <td>" . htmlentities($comentario) . "</td>
                                            <td>$textoDataComentario</td>
                                            <td>
                                                <a href='/new/$refIdNoticia'>"  .  htmlentities($titulo)  . "</a>
                                            </td>
                                            <td class='text-center'>
                                                <div class='btn-group btn-group-xs text-center'>
                                                	<a href='/new/$refIdNoticia' data-toggle='tooltip' title='Ver Comentário' 
                                                	class='text-center btn btn-default'><i class='fa fa-eye'></i></a>
                                                    <a data-href='php/verificacoes/noticia/removerComentario.php?comentario=$idComentariosNoticias' 
                                                    data-toggle='tooltip' title='Eliminar Comentário' class='text-center btn btn-default md-trigger' 
                                                    id='botaoRemover$idComentariosNoticias' data-target='#md-fade-in-scale-up' data-modal='md-fade-in-scale-up' 
                                                    onclick='modalRemover($idComentariosNoticias);'><i class='fa fa-remove'></i></a>
                                                </div>
                                            </td>
                                        </tr>";
                                        }
