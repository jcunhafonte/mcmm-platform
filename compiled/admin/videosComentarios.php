...
<div class='md-modal md-fade-in-scale-up' id='md-fade-in-scale-up'>
    <div class='md-content'>
        ...
        <div>
            <p>Tem a certeza que pretende eliminar o comentário do utilizador <strong><span id='alteravel'></span></strong>?</p>
            <ul>
                <li><strong>Atenção:</strong> Esta ação não pode ser revertida.
                </li>
                <li><strong>Informação:</strong> Após a confirmação da remoção não pode voltar a ver este comentário.
                </li>
...
<div id="wrapper">
    ...
    Menus('videos', 'comentariosVideos');
...
<!--        </ol>-->
        ...
        <div class="content">
            ...
            <div class="page-heading">
                <h1><i class='fa fa-sitemap'></i> Comentários</h1>

                <h3>Visualize os comentários e remova os que pretender</h3></div>
            ...
            <div class="row">
               ...
               <a class='alert-link'>Votos de uma excelente experiência!</a></div>";
                    ...
                    if (isset($_GET['comentarioRemovidoErro'])) {
                        ...
                        <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>
                        Ocorreu um erro ao remover o comentário.<br>
                        <a class='alert-link'>Votos de uma excelente experiência!</a></div>";
                    }
                            ...
                            <div class="table-responsive" style="overflow-x: hidden">
                                <form class='form-horizontal' role='form'>
                                    <table id="datatables-1" class="table table-striped table-bordered" cellspacing="0"
                                        ...
                                        $ativo = 1;
                                        $query = "SELECT comentarios_videos.id_comentarios_videos, comentarios_videos.comentario, 
                                        comentarios_videos.ref_id_utilizador, comentarios_videos.ref_id_videos, 
                                        comentarios_videos.data_comentario, utilizadores.nome_utilizador, utilizadores.id_utilizador, 
                                        utilizadores.id_user, videos.id_videos, videos.ativo, videos.titulo
                                        FROM comentarios_videos
                                        INNER JOIN utilizadores ON utilizadores.id_utilizador = comentarios_videos.ref_id_utilizador
                                        INNER JOIN videos ON videos.id_videos = comentarios_videos.ref_id_videos
                                        WHERE videos.ativo = ?
                                        ORDER BY comentarios_videos.data_comentario";

                                        $stmt = mysqli_prepare($link, $query);
                                        mysqli_stmt_bind_param($stmt, 'i', $ativo);
                                        mysqli_stmt_bind_result($stmt, $idComentariosVideos, $comentario, $refIdUtilizador, $refIdVideo,
                                            $dataComentario, $nomeUtilizador, $idUtilizador, $idUser, $idVideos, $ativo, $titulo);
                                        mysqli_stmt_execute($stmt);

                                        while (mysqli_stmt_fetch($stmt)) {

                                            //Eliminar Horas de Datas
                                            $conversaoDataComentario = new DateTime($dataComentario);
                                            $textoDataComentario = $conversaoDataComentario->format('d-m-Y');

                                            echo "<tr>
                                            <td>
                                                <a href='/@$idUser'>
                                                <span id='titulo$idComentariosVideos'>"  .  htmlentities($nomeUtilizador)  . "</a>
                                                </a>
                                            </td>
                                            <td>" . htmlentities($comentario) . "</td>
                                            <td>$textoDataComentario</td>
                                            <td>
                                                <a href='/video/$refIdVideo'>"  .  htmlentities($titulo)  . "</a>
                                            </td>
                                            <td class='text-center'>
                                                <div class='btn-group btn-group-xs text-center'>
                                                	<a href='/video/$refIdVideo' data-toggle='tooltip' title='Ver Comentário' 
                                                	class='text-center btn btn-default'><i class='fa fa-eye'></i></a>
                                                    <a data-href='php/verificacoes/video/removerComentario.php?comentario=$idComentariosVideos' 
                                                    data-toggle='tooltip' title='Eliminar Comentário' class='text-center btn btn-default md-trigger' 
                                                    id='botaoRemover$idComentariosVideos' data-target='#md-fade-in-scale-up' data-modal='md-fade-in-scale-up' 
                                                    onclick='modalRemover($idComentariosVideos);'><i class='fa fa-remove'></i></a>
                                                </div>
                                            </td>
                                        </tr>";
                                        }
