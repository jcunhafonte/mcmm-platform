...
<div id="wrapper">
    ...
    Menus('projetos', 'comentariosProjetos');
                            ...
                            <div class="table-responsive" style="overflow-x: hidden">
                                <form class='form-horizontal' role='form'>
                                    <table id="datatables-1" class="table table-striped table-bordered" cellspacing="0"
                                        ...
                                        $ativo = 1;
                                        $query = "SELECT comentarios_projetos.id_comentarios_projetos, comentarios_projetos.comentario, 
                                        comentarios_projetos.ref_id_utilizador, comentarios_projetos.ref_id_projeto, 
                                        comentarios_projetos.data_comentario, utilizadores.nome_utilizador, utilizadores.id_utilizador, 
                                        utilizadores.id_user, projetos.id_projetos, projetos.ativo, projetos.titulo
                                        FROM comentarios_projetos
                                        INNER JOIN utilizadores ON utilizadores.id_utilizador = comentarios_projetos.ref_id_utilizador
                                        INNER JOIN projetos ON projetos.id_projetos = comentarios_projetos.ref_id_projeto
                                        WHERE projetos.ativo = ?
                                        ORDER BY comentarios_projetos.data_comentario";

                                        $stmt = mysqli_prepare($link, $query);
                                        mysqli_stmt_bind_param($stmt, 'i', $ativo);
                                        mysqli_stmt_bind_result($stmt, $idComentariosProjetos, $comentario, $refIdUtilizador, $refIdProjeto,
                                            $dataComentario, $nomeUtilizador, $idUtilizador, $idUser, $idProjetos, $ativo, $titulo);
                                        ...
                                        while (mysqli_stmt_fetch($stmt)) {
                                            ...
                                            <td>
                                                <a href='/@$idUser'>
                                                <span id='titulo$idComentariosProjetos'>"  .  htmlentities($nomeUtilizador)  . "</a>
                                                </a>
                                            </td>
                                            <td>" . htmlentities($comentario) . "</td>
                                            <td>$textoDataComentario</td>
                                            <td>
                                                <a href='/project/$refIdProjeto'>"  .  htmlentities($titulo)  . "</a>
                                            </td>
                                            <td class='text-center'>
                                                <div class='btn-group btn-group-xs text-center'>
                                                	<a href='/project/$refIdProjeto' data-toggle='tooltip' title='Ver Comentário' 
                                                	class='text-center btn btn-default'><i class='fa fa-eye'></i></a>
                                                    <a data-href='php/verificacoes/projeto/removerComentario.php?comentario=$idComentariosProjetos' 
                                                    data-toggle='tooltip' title='Eliminar Comentário' class='text-center btn btn-default md-trigger' 
                                                    id='botaoRemover$idComentariosProjetos' data-target='#md-fade-in-scale-up' data-modal='md-fade-in-scale-up' 
                                                    onclick='modalRemover($idComentariosProjetos);'><i class='fa fa-remove'></i></a>
                                                </div>
                                            </td>
                                        </tr>";
                                        }
