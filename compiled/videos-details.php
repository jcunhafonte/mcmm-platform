...
WHERE videos.id_videos = ?");
    ...
    //PREVIOUS
    $result = $conn->prepare("SELECT COUNT(id_videos)
    FROM videos
    WHERE id_videos = (SELECT MAX( id_videos ) 
    FROM videos
    WHERE id_videos < ? AND ativo = 1)");

    $result->bind_param('i', $idVideos);
    $result->execute();
    ...
    if ($numeroResultados > 0) {
        ...
        $result = $conn->prepare("SELECT id_videos, titulo 
    FROM videos
    WHERE id_videos = (SELECT MAX( id_videos ) 
    FROM videos
    WHERE id_videos < ? AND ativo = 1)");
        ...
        $result->bind_param('i', $idVideos);
        $result->execute();
        $result->bind_result($idVideosPrevious, $tituloPrevious);
        $result->fetch();
        $result->close();
        ...
        $result = $conn->prepare("
SELECT id_videos, titulo FROM videos
WHERE ativo = 1 ORDER BY id_videos DESC LIMIT 1");
    ...
    $result = $conn->prepare("SELECT COUNT(id_videos)
    FROM videos
    WHERE id_videos = (SELECT MIN( id_videos ) 
    FROM videos
    WHERE id_videos > ? AND ativo = 1)");
    ...
    if ($numeroResultados > 0) {
        ...
        $result->bind_param('i', $idVideos);
        $result->execute();
        $result->bind_result($idVideosNext, $tituloNext);
        $result->fetch();
        $result->close();
        ...
        $result = $conn->prepare("
        SELECT id_videos, titulo FROM videos
        WHERE ativo = 1 ORDER BY id_videos ASC LIMIT 1");
...
<body data-offset="80" data-target=".navbar" data-spy="scroll" id="video" class="">
...
<div id="platform">
    ...
    <div class="section section-gray">
        ...
        <div class="fixed-title fixed-title-video">
            <div class="container">
                <div class="row">
        ...
        <div class="videos-overview">
            <div class="container">
                <div class="row">
                    ...
                    <div id="video-wrapper" class="col-xs-12">
                        <div class="video" id="video-js">
                            <div id="video-player"></div>
                        </div>
                    </div>

                    <div class="header col-xs-12" id="header-page">
                        <h2>Descrição</h2>
                    </div>
                    <div class="wrapper">
                        <div class="text">
        ...
        <?php user_actions($totalComentarios, $heart, $totalGostos, '', 'red'); ?>

        <?php next_page_video($tituloNext, $idVideosNext) ?>

        <?php comments_video('red', $name); ?>

        <input id="id_video" type="hidden" value="<?php echo $name; ?>"/>
...
<script src="/scripts/videos-details-cd00d07c91.js"></script>
