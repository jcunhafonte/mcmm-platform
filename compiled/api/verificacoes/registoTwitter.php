...
if ($row_number > 0) {
    ...
    $twitterID = $_POST['id_twitter'];
    $validado = 1;

    $result = $conn->prepare("INSERT INTO utilizadores (validado, nome_utilizador, email, data_registo, ultima_visita, id_user, id_twitter)
    VALUES (?, ?, ?, ?, ?, ?, ?)");
    $result->bind_param('issssss', $validado, $nomeUt, $emailUt, $dataRegistoUt, $ultimaVisitaUt, $userID, $twitterID);
    $result->execute();
    $result->close();
