...
if ($row_number > 0) {
    ...
    $googleID = $_POST['id_google'];
    $validado = 1;

    $result = $conn->prepare("INSERT INTO utilizadores (validado, nome_utilizador, email, data_registo, ultima_visita, id_user, id_google)
    VALUES (?, ?, ?, ?, ?, ?, ?)");
    $result->bind_param('issssss', $validado, $nomeUt, $emailUt, $dataRegistoUt, $ultimaVisitaUt, $userID, $googleID);
    $result->execute();
    $result->close();

    $linkUser = "http://mcmm.tech/@" . $userID;
    include_once("../emails/SendGrid/funcoes.php");
    welcomeSocial($nomeUt, $emailUt, $linkUser, 'Google');
