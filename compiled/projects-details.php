...
WHERE projetos.id_projetos = ?");
    ...
    //PREVIOUS
    $result = $conn->prepare("SELECT COUNT(id_projetos)
    FROM projetos
    WHERE id_projetos = (SELECT MAX( id_projetos ) 
    FROM projetos
    WHERE id_projetos < ? AND ativo = 1)");

    $result->bind_param('i', $idProjetos);
    $result->execute();
    ...
    if ($numeroResultados > 0) {
        ...
        $result = $conn->prepare("SELECT id_projetos, titulo 
    FROM projetos
    WHERE id_projetos = (SELECT MAX( id_projetos ) 
    FROM projetos
    WHERE id_projetos < ? AND ativo = 1)");
        ...
        $result->bind_param('i', $idProjetos);
        $result->execute();
        $result->bind_result($idProjetosPrevious, $tituloPrevious);
        $result->fetch();
        $result->close();
        ...
        $result = $conn->prepare("
SELECT id_projetos, titulo FROM projetos WHERE ativo = 1
ORDER BY id_projetos DESC LIMIT 1");
    ...
    $result = $conn->prepare("SELECT COUNT(id_projetos)
    FROM projetos
    WHERE id_projetos = (SELECT MIN( id_projetos ) 
    FROM projetos
    WHERE id_projetos > ? AND ativo = 1)");
    ...
    if ($numeroResultados > 0) {
        ...
        $result = $conn->prepare("SELECT id_projetos, titulo, tipo 
        FROM projetos
        WHERE id_projetos = (SELECT MIN( id_projetos ) 
        FROM projetos
        WHERE id_projetos > ? AND ativo = 1)");
        ...
        $result->bind_param('i', $idProjetos);
        $result->execute();
        $result->bind_result($idProjetosNext, $tituloNext, $tipoNext);
        $result->fetch();
        $result->close();
        ...
        $result = $conn->prepare("
        SELECT id_projetos, titulo, tipo FROM projetos
        WHERE ativo = 1 ORDER BY id_projetos ASC LIMIT 1");
...
<div id="platform">
    ...
    <?php author_card('yellow', $idUtilizador, $idUser, $nomeUtilizador) ?>

    <?php arrows_area('project', 'yellow', $tituloPrevious, $idProjetosPrevious, $tituloNext, $idProjetosNext) ?>

    <div class="news-projects-header">
        <div class="contents" style="background-image: url(/images/backgrounds/projeto_detalhe.gif)">
            <div class="container">
                <div class="row">
    ...
    <div class="section section-gray">
        ...
        <div class="projects-overview">
                ...
                <div class="row">
                    ...
                    <div class="wrapper">
                        <div class="text">
                            ...
                            <div class="col-md-4 col-lg-4 col-sm-12 col-xs-12 topics">
                                <div>
                                    <div class="col-sm-4 col-md-12 nopadding">
                                        <h4>Título</h4>
                                        <p><?php echo h($titulo) ?></p>
                                    </div>
                                    ...
                                    <div class="col-xs-4 col-md-12 nopadding">
                                        <h4>Áreas científicas</h4>
                                        <p><?php echo h($areasCientificas) ?></p>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            if ($tipo == "normal") {
                ...
                echo "
                <div class=\"single-image text-center wow fadeInUp\" data-wow-duration=\"2s\">
                    <img src=\"/api/utilizadores/projetos/$idProjetos.jpg\"/>
        ...
        <?php user_actions($totalComentarios, $heart, $totalGostos, 'user-actions-force-align hidden-lg hidden-md', 'yellow'); ?>

        <?php

        if ($tipoNext == "normal") {
            next_page_projects_normal($tituloNext, $idProjetosNext);
        } else {
            next_page_projects_slider($tituloNext, $idProjetosNext);
        }

        ?>

        <?php comments_projetos('yellow', $name); ?>

        <input id="id_projeto" type="hidden" value="<?php echo $name; ?>"/>
